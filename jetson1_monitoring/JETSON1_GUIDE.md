...
2. 조리사 출근 → **2초** 감지 → MQTT "ON" 신호 전송 + **SSR ON (GPIO HIGH, 로봇 PC 켜짐)**
3. 주간 모드 동안 **SSR 계속 ON 유지** (야간 전환까지)
4. 저녁 19:30 자동 야간 모드 전환
5. 야간 **10분간 사람 없음** → MQTT "OFF" 신호 전송 + **SSR OFF (GPIO LOW, 로봇 PC 꺼짐)**
6. 이후 모션 감지 시 스냅샷 저장
